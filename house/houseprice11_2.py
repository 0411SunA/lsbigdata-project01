...
# 첫 번째 층: ElasticNet, RandomForest, XGBoost, LightGBM, CatBoost
...
# CatBoost
cat_model = CatBoostRegressor(verbose=0)  # verbose=0으로 출력 줄임
param_grid_cat = {
    'learning_rate': [0.01, 0.05, 0.1],
...
grid_search_cat.fit(train_x_scaled, train_y)
...
y5_hat = best_lgbm_model.predict(train_x_scaled)
y6_hat = best_cat_model.predict(train_x_scaled)
...
pred_y_eln2 = best_eln_model2.predict(test_x_scaled)
...
pred_y_cat = best_cat_model.predict(test_x_scaled)
...
# 새로운 모델 성능 평가
pred_y_lgbm_train = best_lgbm_model.predict(train_x_scaled)
pred_y_cat_train = best_cat_model.predict(train_x_scaled)

mse_lgbm = mean_squared_error(train_y, pred_y_lgbm_train)
mse_cat = mean_squared_error(train_y, pred_y_cat_train)

inv_mse_gb = 1 / mse_gb
inv_mse_ridge = 1 / mse_ridge
inv_mse_lasso = 1 / mse_lasso
inv_mse_lgbm = 1 / mse_lgbm
inv_mse_cat = 1 / mse_cat

total_inv_mse = inv_mse_gb + inv_mse_ridge + inv_mse_lasso + inv_mse_lgbm + inv_mse_cat
w_gb = inv_mse_gb / total_inv_mse
w_ridge = inv_mse_ridge / total_inv_mse
w_lasso = inv_mse_lasso / total_inv_mse
w_lgbm = inv_mse_lgbm / total_inv_mse
w_cat = inv_mse_cat / total_inv_mse

pred_y_final = (w_gb * pred_y_gb) + (w_ridge * pred_y_ridge) + (w_lasso * pred_y_lasso) + (w_lgbm * pred_y_lgbm) + (w_cat * pred_y_cat)

# 결과 저장
sub_df["SalePrice"] = pred_y_final

sub_df.to_csv("./data/houseprice/sample_submission15.csv", index=False)
